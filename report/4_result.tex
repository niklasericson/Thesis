...
\FloatBarrier
\section{Comparison}
This section compares and summarizes the stand-alone control approaches (\abbrIRC and \abbrMRACPE) and the approaches that aims to cancel harmonics (\abbrFDC, \abbrRFDC and \abbrIMP).
\begin{table}[h!]
  \centering
  \begin{tabular}{| P{4cm} | P{2.2cm} | P{2.2cm} | P{2.2cm} |}
    \hline
    \bf{Aspect}  & \bf{Present} & \bf{\abbrIRC} & \bf{\abbrMRACPE} \\ \hline
    Closed-loop Bandwidth [Hz] & 9.7 & 73.3 & -\\ \hline
    Tracking error, $\sigma$-[mrads] & $0.29$ & $0.03$ & $0.11$\\ \hline
    Robustness to model errors (model's 1st resonance at 22.0Hz), $\sigma$-[mrads] & $\infty$ & 0.03 & 0.21\\ \hline
    Robustness to model errors (model's 1st resonance at 67.2Hz), $\sigma$-[mrads] & $\infty$ & 0.03 & 0.11\\ \hline
    Disturbance rejection, attenuation-[\%] & $\infty$ & 0.03 & 0.11\\ \hline
    Stability issues & Unstable when model error is to big & Handles model errors better than & Tuning for faster adaption easily leads to instability\\ \hline
    Implementation simplicity & $1$ & $R$ & $1$\\ \hline
  \end{tabular}
...
\begin{table}[h!]
  ...
  \begin{tabular}{| P{4cm} | P{2.2cm} | P{2.2cm} | P{2.2cm} |}
    \hline
      \bf{Aspect} & \bf{\abbrFDC} & \bf{\abbrRFDC} & \bf{\abbrIMP} \\ \hline
    Affecting closed loop system & No & No & Yes\\ \hline
    Cancellation effectiveness of major frequency, attenuation-[\%]                & 64.6 & 88.4 & 83.3\\ \hline
    Robustness to model errors (model's 1st resonance at 28.2Hz), attenuation-[\%] & 40.5 & 26.6 & 73.5\\ \hline
    Robustness to model errors (model's 1st resonance at 43.4Hz), attenuation-[\%] & 57.7 & 5.0 & 83.4\\ \hline
    Implementation simplicity & Requires high order model, computational demanding. & Requires observer, computational demanding. Can be used as add-on.& Controller must be retuned when selecting another frequency. \\ \hline
  \end{tabular}
  \caption{\label{tab:comp} Key parameters for the harmonic cancellation control approaches.}
\end{table}
