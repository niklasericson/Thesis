...
\label{sec:linsys}
The extracted linear dynamics has been identified as an Output-Error system using a (\abbrPRBS) as excitation signal, allowing for a valid extraction. The system transfer function has been derived in discrete-time using the the System Identification Toolbox in Matlab. A more detail description of the procdure is available in \cite{ButcherController:2015}. Figure~\ref{fig:model} shows a comparision with the model in the frequency domain, where the FFT identification is calculated by dividing the FFT of the output with the FFT of the input.  
